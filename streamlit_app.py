...
def fetch_news_sentiment(ticker):
    ...
    return headlines or ["No recent news available."], avg_sent
...
def build_mood_history_df(hist):
    ...
    for i in range(len(hist) - 1):
        ...
        emoji = "🤑" if pct_change > 2 else "😱" if pct_change < -2 else "😐"
...
def altair_mood_chart(df):
    ...
    points = base.mark_text(fontSize=28).encode(
        text='emoji',
        tooltip=[alt.Tooltip('date'), alt.Tooltip('pct_change', format=".2f")]
    )
...
with st.sidebar:
    ...
    user_input = st.text_input("Enter up to 3 tickers (comma-separated):", value=", ".join(st.session_state["tickers"]))
...
st.caption(f"📅 Market is currently: {'🟢 Open' if market_open_now else '🔴 Closed'}")
...
# ---------- Animated Quotron ----------
st.markdown('<div class="ticker-wrap"><div class="ticker"><span>', unsafe_allow_html=True)
for data in mood_results:
    st.markdown(
        f'<a href="#{data["ticker"]}" onclick="window.location.reload();">'
        f'{data["ticker"]} {data["emoji"]} {data["pct_change"]:.2f}%</a>',
        unsafe_allow_html=True
    )
st.markdown('</span></div></div>', unsafe_allow_html=True)

# ---------- Top/Bottom Mood Stocks ----------
sorted_results = sorted(mood_results, key=lambda x: x["score"], reverse=True)
top7 = sorted_results[:7]
bottom7 = sorted_results[-7:]

st.subheader("🏆 Top Mood Stocks")
...
st.subheader("🔻 Lowest Mood Stocks")
